...
/// Possible errors that may be encountered while interacting with a persistent
/// local storage.
...
pub enum Error {
    ...
    #[error("A (de)serialization error occurred")]
...
pub trait PersistedEntity: Serialize + for<'de> Deserialize<'de> {
    ...
    /// Deserializes an instance of [`Self`] from a the contents of
    /// [`Self::FILE_PATH`].
    ///
    /// # Errors
    ///
    ...
    /// Serialize [`self`](Self) into a string and write it to
    /// [`Self::FILE_PATH`].
    ///
    /// # Errors
    ///
