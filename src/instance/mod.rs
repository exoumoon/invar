...
///
/// An instance does NOT take into account the associated `Component`s. Those bounded with an `Instance`,
/// all the `Config`s and `Override`s form a `Pack`. See `Pack` if that's what you're looking for.
...
pub struct Instance {
    ...
    /// Mods with an incompatible loader will be allowed in the pack if this list contains their loader.
    ///
    /// Some mods like [Sinytra Connector](https://github.com/Sinytra/Connector) allow loading Fabric mods
    /// on Forge/NeoForge, this option makes it so you can add mods that would require a compatibility layer
    /// without getting bombarded with incompatibility warnings.
...
pub enum Loader {
    ...
    ///
    /// It is compatible with [`Forge`](Loader::Forge), is open-source and fresh, but is only available for newer versions.
    Neoforge,

    /// The [**Fabric**](https://fabricmc.net) modloader.
    ///
    /// Fabric is a modular, lightweight mod loader for Minecraft. Performant and fresh, but incompatible with [`Forge`](Loader::Forge) mods.
    Fabric,
